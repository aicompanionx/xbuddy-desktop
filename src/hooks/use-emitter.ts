...
export type OnEvents<T = unknown> = Record<string, T>

export type EmitEvents<T = unknown> = Record<string, T>

export type EventHandler<T> = (arg: T) => void

export const useEmitter = <OnEvents, EmitEvents>() => {
  const eventsRef = useRef<Record<string, EventHandler<unknown>[]>>({})
