...
import { checkUrlSafety, clearUrlSafetyCache } from '../api';
...
export const setupUrlSafetyHandlers = (): void => {
    ...
    ipcMain.handle('check-url-safety', async (_event, url: string) => {
        try {
            ...
            if (!result.isSafe) {
                console.warn(`Unsafe URL detected: ${url}, reason: ${result.reason}`);
 ...
 */
function notifyLive2DWindowAboutUnsafeUrl(unsafeResult: any): void {
    try {
        // Get all windows
