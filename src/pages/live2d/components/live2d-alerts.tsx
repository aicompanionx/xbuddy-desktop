...
export const Live2DAlerts = ({ containerRef }: Live2DAlertsProps) => {
  ...
  useEffect(() => {
    if (state.activeAlert && isMenuOpen) {
      setIsMenuOpen(false)
    }
  }, [state.activeAlert, isMenuOpen, setIsMenuOpen])
  ...
  return (
    <>
      ...
      <PhishingAlert
        ...
        isActive={state.activeAlert === 'phishing' && !isMenuOpen}
        onClose={closeAlert}
      ...
      <TokenSafetyAlert
        ...
        isActive={state.activeAlert === 'tokenSafety' && !isMenuOpen}
        referenceElement={containerRef}
        onClose={closeAlert}
      />
